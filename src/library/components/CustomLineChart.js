...
import AbstractChart from 'library/components/AbstractChart';

export default class CustomLineChart extends AbstractChart {
  ...
  renderDots = (config) => {
    ...
    data.map((dataset, index) => {
      dataset.data.map((x, i) => {
        const cx = paddingRight + (i * (width - paddingRight)) / (dataset.data.length - 1);
        const cy = (height / 4)
            * 3
        ...
        <Polygon
          ...
          points={
            `${dataset.data
              .map(
                (d, i) => {
                  const x = paddingRight + (i * (width - paddingRight)) / (dataset.data.length - 1);
  ...
  renderLine = (config) => {
    ...
    data.forEach((dataset, index) => {
      const points = dataset.data.map(
        (d, i) => {
          const x = (i * (width - paddingRight)) / (dataset.data.length - 1) + paddingRight;
          const y = (baseHeight - this.calcHeight(d, datas, height)) / 4 * 3 + paddingTop;
          return `${x},${y}`;
        },
      );
